...
| `status` | `draft` · `accepted` · `rejected`; запись агента — черновик, пока человек не принял её через `cortex review` |
| `shared_with` | профили-совладельцы узла: так легализуется запись в пересечении зон |
| `aliases` | как этот объект называют в переписке — по ним работает `cortex resolve` |
| `disputed` · `contradicts` | узел в споре и с кем; расхождение опускает уверенность обеим сторонам |

Только у ставки (`node_type: bet`, слой `GROUND/DECIDE/bets/`):

| Ключ | Зачем |
|---|---|
| `gate` | ключ гейта из `SCHEMA/gates.yaml`: при каком CP обещание уходит наружу |
| `stage` | 1–6, какой риск снимаем сейчас; к календарю не привязано и с гейтом не путается |
| `threshold` · `due` | порог CP и дата, к которой он должен быть взят |
| `non_critical` | зависимости, которые **не** держат потолок ставки; по умолчанию критические все |
| `outcome` | `met` · `missed` при закрытии — вход `cortex calibrate` |
